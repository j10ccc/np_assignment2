...
struct __attribute__((__packed__)) calcProtocol {
  uint16_t type; // What message is this, 1 = server to client, 2 client to
                 // server, 3... reserved , conversion needed
  uint16_t major_version; // 1, conversion needed
  uint16_t minor_version; // 0, conversion needed
  uint32_t id; // Server side identification with operation. Client must return
               // the same ID as it got from Server., conversion needed
  ...
  double flValue1;  // float value 1,NO NEED TO do host to Network or Network to
                    // Host conversion here, we are using equivalent platforms
  double flValue2;  // float value 2,NO NEED TO do host to Network or Network to
                    // Host conversion here, we are using equivalent platforms
  double flResult;  // float result,NO NEED TO do host to Network or Network to
                    // Host conversion here, we are using equivalent platforms
};
